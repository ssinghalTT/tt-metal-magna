...
def tt_to_hf_shape(key):
    hf_keys = tt_to_hf_key(key)
    if hf_keys is None:
        return None
    if isinstance(hf_keys, list):
        # For multiple keys, concatenate the shapes along the last dimension
        shapes = [hf_state_dict[k].shape for k in hf_keys]
        # Create a new shape that combines the last dimensions
        combined_shape = list(shapes[0][:-1])
        combined_shape.append(sum(shape[-1] for shape in shapes))
        return combined_shape
    else:
        return hf_state_dict[hf_keys].shape


def tt_key_to_hf_value(key, tt_state):
    ...
    # reshape to tt shape (likely just flat)
    tt_shape = np.array(tt_state[key][1]).shape
    hf_value = hf_value.reshape(tt_shape)
    return hf_value
...
# for key in new_keys:
#     value = tt_state[key][1]
#     hf_shape = tt_to_hf_shape(key)
#     if not hf_shape:
#         continue
#     hf_shape = flat_tup(hf_shape)
#     tt_shape = np.array(value).shape
#     assert hf_shape == tt_shape, f"Shape mismatch for key '{key}': HF shape {hf_shape}, TT shape {tt_shape}"

#     # check the value against the hf model
#     hf_value = tt_key_to_hf_value(key, tt_state)
#     tt_value = np.array(value)
#     assert np.allclose(hf_value, tt_value), f"Value mismatch for key '{key}': HF value {hf_value}, TT value {tt_value}"
