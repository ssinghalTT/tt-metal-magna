...
class Mv2Like(GstBase.BaseTransform):
    # Element metadata (for GStreamer)

    __gtype_name__ = "GstMv2LikePythonBatching"

    __gstmetadata__ = (
        "Mv2Like Python",  # Long name
    ...
    #    self.model = Mv2LikeTrace2CQ()
    #    print("########################################", self.batch_size)
    #    self.model.initialize_mv2like_trace_2cqs_inference(self.device,self.batch_size)

    # except Exception as e:
    #    Gst.exception_object(self, "Mv2Like: CRITICAL ERROR DURING __init__", exc=e)
    ...
    def initialize_device(self, batch_size):
        ...
        self.model = Mv2LikeTrace2CQ()
        print("########################################", batch_size)
        self.model.initialize_mv2like_trace_2cqs_inference(self.device, batch_size)
    ...
    def do_transform(self, inbuf: Gst.Buffer, outbuf: Gst.Buffer) -> Gst.FlowReturn:
        ...
        try:
            ...
            if not success:
                Gst.error("Mv2Like: Failed to map input buffer")
            ...
            if not success:
                Gst.error("Mv2Like: Failed to map output buffer for writing")
                return Gst.FlowReturn.ERROR
            ...
            Gst.error(f"Mv2Like: Error in transform: {e}")
...
# After this call, Mv2Like.get_type() would be available.
GObject.type_register(Mv2Like)
