...
from models.demos.wormhole.deit_tiny.demo.deit_e2e_performant import DeitTrace2CQ
from ttnn.model_preprocessing import preprocess_model_parameters
from models.utility_functions import is_wormhole_b0, torch2tt_tensor, is_blackhole

# from models.experimental.deit.performant_files.deit_test_infra import create_test_infra
...
class Deit(GstBase.BaseTransform):
    # Element metadata (for GStreamer)

    __gtype_name__ = "GstDeitPythonBatching"

    __gstmetadata__ = (
        "Deit Python",  # Long name
    ...
    __gproperties__ = {
        ...
        # Controls how often stats are printed (in milliseconds). Defaults to 500ms to match fpsdisplaysink.
        "stats-interval-ms": (
            int,
            "Stats interval (ms)",
    ...
    #    self.model.initialize_deit_trace_2cqs_inference(self.device,self.batch_size)

    # except Exception as e:
    #    Gst.exception_object(self, "Deit: CRITICAL ERROR DURING __init__", exc=e)
    ...
    def initialize_device(self, batch_size):
        ...
        self.model = DeitTrace2CQ()
        print("########################################", batch_size)
        self.model.initialize_deit_trace_2cqs_inference(self.device, batch_size)
    ...
    def do_transform(self, inbuf: Gst.Buffer, outbuf: Gst.Buffer) -> Gst.FlowReturn:
        ...
        try:
            ...
            if not success:
                Gst.error("Deit: Failed to map input buffer")
            ...
            t1 = time.time()
            out = self.model.run_trace_2cqs_inference(frame_data)
            t2 = time.time()
            ...
            if not success:
                Gst.error("Deit: Failed to map output buffer for writing")
                return Gst.FlowReturn.ERROR
            ...
            Gst.error(f"Deit: Error in transform: {e}")
            try:
...
# After this call, Deit.get_type() would be available.
GObject.type_register(Deit)
