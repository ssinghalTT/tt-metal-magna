    ...
    bind_registered_operation(
        ...
        ttnn::pybind_overload_t{
            [](const data_movement_operation_t& self,
               const ttnn::Tensor& input_tensor,
               const int& shifts,
               const int& dim) -> ttnn::Tensor { return self(input_tensor, shifts, dim); },
            py::arg("input_tensor"),
            py::arg("shifts"),
            py::arg("dim")},
...
void py_bind_roll(pybind11::module& module) {
    detail::bind_roll(
        ...
        Args:
            ...
            dim: The dimension(s) along which to roll. Must have the same length as `shifts` if a list is provided.
        )doc");
}
