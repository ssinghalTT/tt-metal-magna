...
JitBuildIdleEthernet::JitBuildIdleEthernet(const JitBuildEnv& env, const JitBuiltStateConfig& build_config) :
    ...
    switch (this->core_id_) {
        ...
        case 1: {
            this->target_name_ = "drone_idle_erisc";
            this->cflags_ = env_.cflags_ + "-fno-tree-loop-distribute-patterns ";  // don't use memcpy for cpy loops
            this->defines_ +=
            ...
            this->includes_ += "-I " + env_.root_ + "tt_metal/hw/firmware/src ";
            if (this->is_fw_) {
                this->srcs_.push_back("tt_metal/hw/firmware/src/drone_idle_erisc.cc");
            } else {
                this->srcs_.push_back("tt_metal/hw/firmware/src/idle_erisck.cc");
            }
            if (this->is_fw_) {
                this->lflags_ +=
                    "-T" + env_.root_ + "runtime/hw/toolchain/" + get_alias(env_.arch_) + "/firmware_drone_ierisc.ld ";
            } else {
                this->lflags_ +=
                    "-T" + env_.root_ + "runtime/hw/toolchain/" + get_alias(env_.arch_) + "/kernel_drone_ierisc.ld ";
            }
            break;
        }
