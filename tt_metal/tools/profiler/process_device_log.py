...
def get_dispatch_core_ops(timeseries):
    ...
    riscData = {
        ...
        droneRisc: {"zone": [], "opID": 0, "cmdType": "", "ops": {}, "orderedOpIDs": [], "opFinished": False},
    }
    for ts in timeseries:
        timerID, tsValue, attachedData, risc = ts
        ...
        if "meta_data" in timerID and "dispatch_command_type" in timerID["meta_data"]:
            ...
            if "CQ_DISPATCH_NOTIFY_DRONE_GO_SIGNAL" in riscData[risc]["cmdType"]:
    ...
    opsDict = {}
    for masterOpID, droneOpID in zip(riscData[masterRisc]["orderedOpIDs"], riscData[droneRisc]["orderedOpIDs"]):
        opsDict[masterOpID] = riscData[masterRisc]["ops"][masterOpID] + riscData[droneRisc]["ops"][droneOpID]
