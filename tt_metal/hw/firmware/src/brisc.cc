...
inline void run_triscs(dispatch_core_processor_masks enables) {
    ...
    while (mailboxes->drone_sync.trisc0 != RUN_SYNC_MSG_DONE) {
        invalidate_l1_cache();
    }

    if (enables & DISPATCH_CLASS_MASK_TENSIX_ENABLE_COMPUTE) {
        mailboxes->drone_sync.trisc0 = RUN_SYNC_MSG_GO;
        mailboxes->drone_sync.trisc1 = RUN_SYNC_MSG_GO;
        mailboxes->drone_sync.trisc2 = RUN_SYNC_MSG_GO;
    }
}
...
inline void start_ncrisc_kernel_run_early(dispatch_core_processor_masks enables) {
    ...
    if (enables & DISPATCH_CLASS_MASK_TENSIX_ENABLE_DM1) {
        mailboxes->drone_sync.dm1 = RUN_SYNC_MSG_GO;
    }
#endif
}
...
inline void start_ncrisc_kernel_run(dispatch_core_processor_masks enables) {
    ...
    if (enables & DISPATCH_CLASS_MASK_TENSIX_ENABLE_DM1) {
        ...
        // address it provides.
        while (mailboxes->drone_sync.dm1 != RUN_SYNC_MSG_WAITING_FOR_RESET);
        mailboxes->drone_sync.dm1 = RUN_SYNC_MSG_GO;
        volatile tt_reg_ptr uint32_t* cfg_regs = core.cfg_regs_base(0);
...
inline void wait_ncrisc_trisc() {
    WAYPOINT("NTW");
    while (mailboxes->drone_sync.all != RUN_SYNC_MSG_ALL_DRONES_DONE) {
...
inline void trigger_sync_register_init() { mailboxes->drone_sync.trisc0 = RUN_SYNC_MSG_INIT_SYNC_REGISTERS; }
...
int main() {
    ...
    while (1) {
...

        {
            ...
            if (enables & DISPATCH_CLASS_MASK_TENSIX_ENABLE_DM1) {
                mailboxes->drone_sync.dm1 = RUN_SYNC_MSG_LOAD;
            }
