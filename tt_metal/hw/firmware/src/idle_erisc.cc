...
void set_deassert_addresses() {
    ...
    WRITE_REG(DRONE_IERISC_RESET_PC, MEM_DRONE_IERISC_FIRMWARE_BASE);
#endif
}
...
inline void run_drone_eriscs(dispatch_core_processor_masks enables) {
    if (enables & DISPATCH_CLASS_MASK_ETH_DM1) {
        mailboxes->drone_sync.dm1 = RUN_SYNC_MSG_GO;
    }
}

inline void wait_drone_eriscs(uint32_t &heartbeat) {
    WAYPOINT("SEW");
