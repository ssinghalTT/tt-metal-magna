...
volatile tt_l1_ptr uint8_t *const drone_idle_erisc_run = &mailboxes->drone_sync.dm1;
...
inline __attribute__((always_inline)) void signal_drone_idle_erisc_completion() {
    *drone_idle_erisc_run = RUN_SYNC_MSG_DONE;
}
...
int main(int argc, char *argv[]) {
    ...
    DIRTY_STACK_MEMORY();
    WAYPOINT("I");
    do_crt1((uint32_t *)MEM_DRONE_IERISC_INIT_LOCAL_L1_BASE_SCRATCH);
    ...
    while (1) {
        WAYPOINT("W");
        while (*drone_idle_erisc_run != RUN_SYNC_MSG_GO) {
            invalidate_l1_cache();
        }
        DeviceZoneScopedMainN("DRONE-IDLE-ERISC-FW");
        ...
        signal_drone_idle_erisc_completion();
    }
