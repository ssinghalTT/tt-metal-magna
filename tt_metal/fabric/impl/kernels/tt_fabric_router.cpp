...
bool terminated_drone_routers = false;
...
void kernel_main() {
    ...
    if constexpr (is_master) {
        ...
        notify_drone_routers(router_mask, master_router_chan, FABRIC_ROUTER_SYNC_SEM, sync_val);
    ...
    while (1) {
        ...
        if ((loop_count & SWITCH_THRESHOLD) == SWITCH_THRESHOLD) {
            ...
            if (*(volatile uint32_t*)FABRIC_ROUTER_SYNC_SEM == 0) {
                ...
                if constexpr (is_master) {
                    if (!terminated_drone_routers) {
                        notify_drone_routers(router_mask, master_router_chan, FABRIC_ROUTER_SYNC_SEM, 0);
                        terminated_drone_routers = true;
                    }
                }
