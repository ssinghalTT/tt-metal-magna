...
FORCE_INLINE void send_next_data(
    ...
    remote_update_ptr_val<DEFAULT_ETH_TXQ, to_receiver_pkts_sent_id>(words_to_forward);
}
...
FORCE_INLINE void establish_connection(
    ...
    local_sender_channel_worker_interface.template update_worker_copy_of_free_slots<enable_ring_support>(
        get_ptr_val(stream_id) - local_sender_channel_worker_interface.worker_location_info_ptr->edm_read_counter);
}
...
FORCE_INLINE void check_worker_connections(
    ...
    if (!channel_connection_established) {
        ...
        if (connect_is_requested(cached)) {
            ...
            establish_connection(local_sender_channel_worker_interface, stream_id);
        }
        ...
        local_sender_channel_worker_interface.template teardown_connection<true>(get_ptr_val(stream_id));
    }
}
    ...
    if (completions_since_last_check) {
        ...
        if constexpr (!enable_first_level_ack) {
            if constexpr (SKIP_CONNECTION_LIVENESS_CHECK) {
                ...
                if (channel_connection_established) {
                    local_sender_channel_worker_interface
                        .template update_worker_copy_of_free_slots<enable_ring_support>(completions_since_last_check);
                }
            }
        }
    }
    ...
    *edm_status_ptr = tt::tt_fabric::EDMStatus::STARTED;
    ...
    if (has_downstream_edm_vc0_buffer_connection) {
        new (&downstream_edm_noc_interfaces[0]) tt::tt_fabric::EdmToEdmSender<SENDER_NUM_BUFFERS>(
            ...
            sender_channel_1_free_slots_stream_id,
            receiver_channel_0_free_slots_stream_id,
    ...
    if constexpr (enable_ring_support) {
        if (has_downstream_edm_vc1_buffer_connection) {
            new (&downstream_edm_noc_interfaces[1]) tt::tt_fabric::EdmToEdmSender<SENDER_NUM_BUFFERS>(
                ...
                sender_channel_2_free_slots_stream_id,
                receiver_channel_1_free_slots_stream_id,
