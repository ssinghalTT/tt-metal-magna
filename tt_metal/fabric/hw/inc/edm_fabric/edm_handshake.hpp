 ...
 * the drone. The handshake is broken into 2 parts so that the master can initiate the handshake
 ...
 *
 * Note that the master and drone concepts here only apply in the context of handshaking and initialization
...
/*
 * Initialize base datastructures and values which are common to master and drone EDM cores.
...
/*
 * As the designated master EDM core, wait for the acknowledgement from the drone EDM core
 */
...
/*
 * Return: true if drone EDM handshake core is able to complete the handshake with
 * an ack.
 */
FORCE_INLINE bool receiver_side_can_finish() { return eth_bytes_are_available_on_channel(0); }

/*
 * As the designated drone EDM core, send the acknowledgement to the master EDM core.
 * The drone EDM core shall only acknowledge after receiving the initial handshake packet
