...
HalCoreInfoType create_active_eth_mem_map() {
    ...
    for (std::size_t processor_class_idx = 0; processor_class_idx < processor_classes.size(); processor_class_idx++) {
        ...
        processor_types[static_cast<std::size_t>(EthProcessorTypes::DM1)] = HalJitBuildConfig{
            .fw_base_addr = MEM_SLAVE_AERISC_FIRMWARE_BASE,
            .local_init_addr = MEM_SLAVE_AERISC_INIT_LOCAL_L1_BASE_SCRATCH,
            .fw_launch_addr = SLAVE_AERISC_RESET_PC,
            .fw_launch_addr_value = MEM_SLAVE_AERISC_FIRMWARE_BASE};
