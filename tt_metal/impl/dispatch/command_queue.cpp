...
void HWCommandQueue::enqueue_program(Program& program, bool blocking) {
    ...
    auto command = EnqueueProgramCommand(
        ...
        this->device->worker_launch_message_buffer_state[sub_device_index].get_mcast_wptr(),
        this->device->worker_launch_message_buffer_state[sub_device_index].get_unicast_wptr(),
        sub_device_id);
    // Update wptrs for tensix and eth launch message in the device class
    if (program.runs_on_noc_multicast_only_cores()) {
        this->device->worker_launch_message_buffer_state[sub_device_index].inc_mcast_wptr(1);
    }
    if (program.runs_on_noc_unicast_only_cores()) {
        this->device->worker_launch_message_buffer_state[sub_device_index].inc_unicast_wptr(1);
    }
    this->enqueue_command(command, blocking, sub_device_ids);
