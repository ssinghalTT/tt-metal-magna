...
void HWCommandQueue::enqueue_program(Program& program, bool blocking) {
    ...
    auto command = EnqueueProgramCommand(
        ...
        this->device->worker_launch_message_buffer_state[sub_device_id.to_index()].get_mcast_wptr(),
        this->device->worker_launch_message_buffer_state[sub_device_id.to_index()].get_unicast_wptr(),
        sub_device_id);
    // Update wptrs for tensix and eth launch message in the device class
    if (program.runs_on_noc_multicast_only_cores()) {
        this->device->worker_launch_message_buffer_state[sub_device_id.to_index()].inc_mcast_wptr(1);
    }
    if (program.runs_on_noc_unicast_only_cores()) {
        this->device->worker_launch_message_buffer_state[sub_device_id.to_index()].inc_unicast_wptr(1);
    }
    this->enqueue_command(command, blocking, sub_device_ids);
...
void HWCommandQueue::enqueue_trace(const uint32_t trace_id, bool blocking) {
    ...
    for (const auto& [id, desc]: trace_inst->desc->descriptors) {
        ...
        if (desc.num_traced_programs_needing_go_signal_multicast) {
            this->device->worker_launch_message_buffer_state[id.to_index()].set_mcast_wptr(desc.num_traced_programs_needing_go_signal_multicast);
        }
        if (desc.num_traced_programs_needing_go_signal_unicast) {
            this->device->worker_launch_message_buffer_state[id.to_index()].set_unicast_wptr(desc.num_traced_programs_needing_go_signal_unicast);
        }
