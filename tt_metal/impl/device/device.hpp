...
class Device {
   ...
   public:
    ...
    const Allocator &get_initialized_allocator() const;
    const Allocator &get_initialized_allocator(SubDeviceId sub_device_id) const;
    Allocator &get_initialized_allocator();
    ...
    std::array<LaunchMessageRingBufferState, detail::SubDeviceManager::MAX_NUM_SUB_DEVICES> worker_launch_message_buffer_state;
    uint8_t num_hw_cqs_;
