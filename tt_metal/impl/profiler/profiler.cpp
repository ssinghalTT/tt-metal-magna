...
public:
    ...
    FabricRoutingLookup(EthCoreToChannelMap&& eth_map, RoutingPathMap&& routing_map) :
        eth_core_to_channel_lookup_(std::move(eth_map)), routing_path_map_(std::move(routing_map)) {}
...
FabricRoutingLookup DeviceProfiler::buildFabricRoutingLookup(const IDevice* device) const {
    ...
    for (chip_id_t chip_id_src : physical_chip_ids) {
        ...
        for (const auto& [_, eth_channels] : active_fabric_eth_channels) {
            for (const auto& eth_chan : eth_channels) {
                ...
                eth_core_to_channel_lookup.emplace(std::make_tuple(chip_id_src, eth_physical_core), eth_chan);
                log_info(
                    "Device {} Eth Chan {} : {},{}", chip_id_src, eth_chan, eth_physical_core.x, eth_physical_core.y);
            }
        }
    ...
    // PHASE II - Build routing path map (precomputed route set including all forwarding for any chip to chip transfer)
    FabricRoutingLookup::RoutingPathMap routing_path_map;
    for (chip_id_t chip_id_src : physical_chip_ids) {
        if (device->is_mmio_capable() &&
            (tt::tt_metal::MetalContext::instance().get_cluster().get_cluster_type() == tt::ClusterType::TG)) {
            // skip lauching on gateways for TG
        ...
        for (chip_id_t chip_id_dst : physical_chip_ids) {
            ...
            for (auto direction : routing_directions) {
                for (auto eth_chan : active_fabric_eth_channels[{chip_id_src, direction}]) {
                    ...
                    auto src_router_eth_core = src_soc_desc.get_eth_core_for_channel(eth_chan, CoordSystem::PHYSICAL);
                    ...
                    for (size_t i = 0; i < route.size(); ++i) {
                        ...
                        if (i == route.size() - 1) {
                            ...
                            FabricRoutingLookup::ForwardingRoute forwarding_route = {route_eth_physical_core, {0, 0}};
                    ...
                    routing_path_map[std::make_tuple(chip_id_src, src_router_eth_core, hop_count)] = all_routes;
                }
            }
        }
    }
    ...
    return FabricRoutingLookup(std::move(eth_core_to_channel_lookup), std::move(routing_path_map));
}
...
void DeviceProfiler::dumpResults(
    ...
    if (state == ProfilerDumpState::NORMAL && rtoptions.get_profiler_noc_events_enabled()) {
        routing_lookup = buildFabricRoutingLookup(device);
    }
