...
const char* get_riscv_name(const CoreCoord& core, uint32_t type) {
    switch (type) {
        ...
        case DebugDroneIErisc: return "drone_ierisc";
...
void WatcherDeviceReader::DumpLaunchMessage(CoreDescriptor& core, const mailboxes_t* mbox_data) {
    ...
    const launch_msg_t* launch_msg = get_valid_launch_message(mbox_data);
    const drone_sync_msg_t* drone_sync = &mbox_data->drone_sync;
    fprintf(f, "rmsg:");
    ...
    if (!is_eth) {
        fprintf(f, "dmsg:");
        DumpRunState(core, launch_msg, drone_sync->dm1);
        DumpRunState(core, launch_msg, drone_sync->trisc0);
        DumpRunState(core, launch_msg, drone_sync->trisc1);
        DumpRunState(core, launch_msg, drone_sync->trisc2);
        fprintf(f, " ");
