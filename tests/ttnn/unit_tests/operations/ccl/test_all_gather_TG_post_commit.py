...
def run_line_all_gather_on_TG_with_full_mesh(
    mesh_device,
    num_devices,
    per_chip_output_shape,
    tensor_memory_layout,
    ...
    full_mesh_input_shape = list(per_chip_output_shape)
    full_mesh_input_shape[dim] *= num_all_gather_instances

    logger.info(f"per_chip_output_shape: {full_mesh_input_shape}, dim: {dim}, " f"num_devices: {num_devices}")
    ...
    # Create the tensor using full mesh shape
    ttnn_tensor = ttnn.from_torch(
        torch.rand(full_mesh_input_shape, dtype=torch.bfloat16),
        tile=ttnn.Tile(tile),
        dtype=input_dtype,
        device=mesh_device,
        layout=layout,
        memory_config=ttnn.MemoryConfig(tensor_memory_layout, buffer_type=buffer_type, shard_spec=input_shard_spec),
    ...
    tt_output_tensor = ttnn.to_torch(
        ttnn_tensor_out,
        mesh_composer=ConcatMeshToTensor(mesh_device, dim=dim),
    )
    ...
    output_golden[:, :, :, :] = torch.rand(full_mesh_input_shape, dtype=torch.bfloat16).repeat(repeat_factor)
...
@pytest.mark.parametrize(
    "num_devices, num_links, per_chip_output_shape, dim, layout",
    [
        (8 * 4, 4, [1, 8, 32, 256], 3, ttnn.TILE_LAYOUT),
        # (8, 4, [8, 1, 32, 16384 * 4], 0, ttnn.TILE_LAYOUT),
        # (4, 3, [1, 4, 32, 2304], 1, ttnn.TILE_LAYOUT),
        # (4, 3, [1, 4, 32, 4096], 1, ttnn.TILE_LAYOUT),
        # (4, 3, [1, 4, 32, 6656], 1, ttnn.TILE_LAYOUT),
    ],
...
@pytest.mark.parametrize("mesh_device", [pytest.param((8, 4), id="8x4_grid")], indirect=True)
def test_line_all_gather_on_TG_full_mesh_post_commit(
    mesh_device,
    num_devices,
    per_chip_output_shape,
    dim,
    ...
    run_line_all_gather_on_TG_with_full_mesh(
        mesh_device,
        num_devices,
        per_chip_output_shape,
        ttnn.TensorMemoryLayout.INTERLEAVED,
