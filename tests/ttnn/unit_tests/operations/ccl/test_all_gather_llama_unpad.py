...
def run_all_gather_unpad(
    ...
    for i, t in enumerate(ttnn.get_device_tensors(tt_out_tensor)):
        ...
        # for start in range(0, 32, input_shape[2]):
        #     eq_debug, output_debug = comp_equal(
        #         tt_output_tensor[:, :, start : start + input_shape[2]],
        #         unchunked_input_tensor[:, :, start : start + input_shape[2]],
        #     )
        #     logger.info(f"chunk {start} to {start+input_shape[2]}: {output_debug}")
