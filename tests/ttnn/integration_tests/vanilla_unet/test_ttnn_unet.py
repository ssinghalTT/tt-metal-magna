        ...
        if isinstance(model, UNet):
            for i in range(1, 5):
                ...
                parameters[f"encoder{i}"][0]["bias"] = ttnn.from_torch(
                    torch.reshape(conv_bias, (1, 1, 1, -1)), dtype=ttnn.bfloat16  # ,layout=ttnn.TILE_LAYOUT
                )
            ...
            parameters["conv"]["bias"] = ttnn.from_torch(
                torch.reshape(model.conv.bias, (1, 1, 1, -1)), dtype=ttnn.bfloat16  # ,layout=ttnn.TILE_LAYOUT
            )
