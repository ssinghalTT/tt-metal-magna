...
std::vector<uint32_t> generate_rt_args(
    uint32_t master_semaphore, uint32_t drone_semaphore, std::vector<std::shared_ptr<Buffer>> const& data_buffers) {
    std::vector<uint32_t> rt_args;
    rt_args.reserve(2 + n_cbs);
    rt_args.push_back(master_semaphore);
    rt_args.push_back(drone_semaphore);
...
TEST_F(DeviceFixture, TensixTestCircularBufferNonBlockingAPIs) {
    ...
    auto const drone_semaphore = CreateSemaphore(program, worker_core, 0, CoreType::WORKER);
    ...
    auto const& drone_data_buffers = create_output_buffers(program, device);
    ...
    auto const drone_kernel_id = tt::tt_metal::CreateKernel(
        program,
        "tests/tt_metal/tt_metal/test_kernels/misc/circular_buffer/cb_non_blocking_drone_test_kernel.cpp",
        worker_core,
        tt::tt_metal::WriterDataMovementConfig{kernel_ct_args});

    auto const& master_rt_args = generate_rt_args(master_semaphore, drone_semaphore, master_data_buffers);
    auto const& drone_rt_args = generate_rt_args(master_semaphore, drone_semaphore, drone_data_buffers);

    tt::tt_metal::SetRuntimeArgs(program, master_kernel_id, worker_core, master_rt_args);
    tt::tt_metal::SetRuntimeArgs(program, drone_kernel_id, worker_core, drone_rt_args);
    ...
    for (size_t i = 0; i < n_cbs; i++) {
        tt::tt_metal::detail::ReadFromBuffer(drone_data_buffers[i], out_buf, true);
