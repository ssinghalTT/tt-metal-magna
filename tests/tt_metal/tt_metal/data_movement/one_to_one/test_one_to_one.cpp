...
struct OneToOneConfig {
    ...
    CoreCoord drone_core_coord = CoreCoord();
...
bool run_dm(IDevice* device, const OneToOneConfig& test_config) {
    ...
    CoreRangeSet drone_core_set({CoreRange(test_config.drone_core_coord)});
    ...
    auto drone_shard_parameters = ShardSpecBuffer(
        drone_core_set,
        {1, total_size_bytes / 2},
        ShardOrientation::ROW_MAJOR,
        {1, test_config.page_size_bytes / 2},
        {1, total_size_pages});
    auto drone_l1_buffer = CreateBuffer(ShardedBufferConfig{
        .device = device,
        .size = total_size_bytes,
        .page_size = test_config.page_size_bytes,
        .buffer_type = BufferType::L1,
        .buffer_layout = TensorMemoryLayout::WIDTH_SHARDED,
        .shard_parameters = std::move(drone_shard_parameters),
    });
    uint32_t drone_l1_byte_address = drone_l1_buffer->address();

    // Compile-time arguments for kernels
    vector<uint32_t> sender_compile_args = {
        (uint32_t)master_l1_byte_address,
        (uint32_t)drone_l1_byte_address,
    ...
    // Semaphores
    CoreRangeSet sem_core_set = drone_core_set.merge<CoreRangeSet>(master_core_set);
    const uint32_t sem_id = CreateSemaphore(program, sem_core_set, 0);
    CoreCoord physical_drone_core = device->worker_core_from_logical_core(test_config.drone_core_coord);

    // Runtime Arguments
    SetRuntimeArgs(program, sender_kernel, master_core_set, {sem_id, physical_drone_core.x, physical_drone_core.y});
    SetRuntimeArgs(program, receiver_kernel, drone_core_set, {sem_id});
    ...
    detail::ReadFromBuffer(drone_l1_buffer, packed_output);
...
TEST_F(DeviceFixture, TensixDataMovementOneToOnePacketSizes) {
    ...
    CoreCoord drone_core_coord = {1, 1};
    ...
    for (uint32_t num_of_transactions = 1; num_of_transactions <= max_transactions; num_of_transactions *= 2) {
        for (uint32_t transaction_size_pages = 1; transaction_size_pages <= max_transaction_size_pages;
            ...
            unit_tests::dm::core_to_core::OneToOneConfig test_config = {
                ...
                .drone_core_coord = drone_core_coord,
