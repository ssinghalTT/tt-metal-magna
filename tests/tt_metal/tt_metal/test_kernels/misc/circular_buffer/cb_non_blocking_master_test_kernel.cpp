...
void kernel_main() {
    ...
    auto drone_sem_addr = reinterpret_cast<volatile uint32_t*>(get_semaphore(get_arg_val<uint32_t>(arg_idx++)));
    ...
    for (int32_t i = 0; i < n_cbs; i++) {
        ...
        for (int32_t j = 0; j < n_pages; j++) {
            ...
            noc_semaphore_wait(drone_sem_addr, 1);
            // noc_semaphore_set(drone_sem_addr, 0);
            ...
            noc_semaphore_wait(drone_sem_addr, 2);
            noc_semaphore_set(drone_sem_addr, 0);
