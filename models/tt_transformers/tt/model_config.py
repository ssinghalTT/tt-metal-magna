    ...
    def accuracy(cls, model_name):
        ...
        if base_model_name in ["Llama3.1-70B", "Llama3.2-90B", "DeepSeek-R1-Distill-Llama-70B", "Qwen2.5-72B"]:
            ...
            if model_name.startswith("Llama3"):
                logger.info(
                    f"Llama 3 models test insensitive to attention precision, using BFP8 attention and kv-cache with FP16 MLP accumulation even in accuracy mode"
                )
                inst = cls(
                    {
                        "TensorPrecision": {
...
class ModelArgs:
    ...
    def __init__(
        ...
        if LLAMA_DIR:
            ...
            self.model_name = HF_MODEL.split("/")[-1]  # May be overridden by config
