...
from models.experimental.mv2like.tests.mv2like_e2e_performant import Mv2LikeTrace2CQ
...
def test_run_mv2like_trace_2cq_inference(
    device,
    use_program_cache,
    batch_size,
    model_location_generator,
):
    mv2like_trac2_2cq = Mv2LikeTrace2CQ()

    mv2like_trac2_2cq.initialize_mv2like_trace_2cqs_inference(
        device,
        batch_size,
    ...
    mv2like_trac2_2cq.release_mv2like_trace_2cqs_inference()
    inference_time_avg = round(sum(inference_time_iter) / len(inference_time_iter), 6)
    logger.info(
        f"ttnn_mmv2like_224x224_batch_size_{batch_size}. One inference iteration time (sec): {inference_time_avg}, FPS: {round(batch_size/inference_time_avg)}"
    )
