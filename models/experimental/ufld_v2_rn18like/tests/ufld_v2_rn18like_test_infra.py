...
def load_torch_model():
    torch_model = TuSimple18like(input_height=320, input_width=800)
    torch_model.eval()
    return torch_model
...
class UFLDv2rn18TestInfra:
    def __init__(
        ...
        self.ttnn_ufld_v2_rn18__model = load_ttnn_model(self.device, torch_model, self.torch_input_tensor)
        self.tt_input_tensor = ttnn.from_torch(self.torch_input_tensor, ttnn.bfloat16)
        self.torch_output_tensor_1, self.torch_output_tensor_2 = torch_model(self.torch_input_tensor)

    def run(self):
        self.output_tensor_1 = self.ttnn_ufld_v2_rn18__model(input=self.input_tensor, batch_size=self.batch_size)
    ...
    def validate(self, output_tensor=None):
        ...
        logger.info(f"ufld_v2_rn18like batch_size={self.batch_size}, PCC={self.pcc_message}")
