...
class TuSimple18like(nn.Module):
    def __init__(self, input_height=320, input_width=800):
        super(TuSimple18like, self).__init__()
        ...
        self.fc_norm = False
        self.model = CustomResNet18like(BasicBlock, [2, 2, 2, 2])
    ...
    def forward(self, x):
        ...
        fea = self.pool(fea)
        out = self.cls[1](fea)
        out = self.cls[2](out).squeeze(-1).squeeze(-1)
        out = self.cls[3](out)
        out = self.cls[4](out)
        pred_dict = {
