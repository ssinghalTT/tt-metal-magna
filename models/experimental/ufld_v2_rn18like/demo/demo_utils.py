...
def overlay_lanes(image_path, res, output_path):
    # Load image
    ...
    # Convert h_samples to numpy array for convenience
    h_samples = np.array(h_samples)

    # Colors for lanes (you can add more if needed)
    colors = [(0, 255, 0), (0, 0, 255), (255, 0, 0)]  # green, red, blue

    # Thickness of the lane lines
    thickness = 3
    ...
    for lane_idx, lane_xs in enumerate(lanes):
        ...
        # Optionally, draw circles on points
        for x, y in points:
            cv2.circle(img, (x, y), radius=5, color=colors[lane_idx % len(colors)], thickness=-1)
...
class LaneTestDatasetFromFolder(torch.utils.data.Dataset):
    def __init__(self, path, img_transform=None, crop_size=None):
        super(LaneTestDatasetFromFolder, self).__init__()
        self.path = path
        self.img_transform = img_transform
        self.crop_size = crop_size
        self.image_files = [
...
def get_inference_loader(batch_size, data_root, dataset, distributed, crop_ratio, train_width, train_height):
    if dataset == "Tusimple":
        img_transforms = transforms.Compose(
            [
                transforms.Resize((int(train_height / crop_ratio), train_width)),
        ...
        test_dataset = LaneTestDatasetFromFolder(
...
def run_inference_tusimple(
    ...
    loader = get_inference_loader(batch_size, data_root, "Tusimple", distributed, crop_ratio, train_width, train_height)

    os.makedirs(os.path.join(data_root, "predict_results"), exist_ok=True)
    ...
    for data in tqdm(loader, desc="Processing images", ncols=100):
        ...
        overlay_lanes(
            os.path.join(data_root, "predict", name), tmp_dict, os.path.join(data_root, "predict_results", name)
        )
