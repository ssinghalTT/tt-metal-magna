...
class SemanticSegmentationDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        inputs, _ = self.image_processor(TF.to_tensor(cv2.imread(str(image_path))), TF.to_tensor(cv2.imread(str(mask_path), cv2.IMREAD_GRAYSCALE)))
...
def test_demo_semantic_segmentation(device):
    weights_path = "models/experimental/lraspp/lraspp_mobilenet_v2_trained_statedict.pth"  # specify your weights path here
    ...
    for batch in tqdm(data_loader, desc="Processing batches"):
        ...
        ttnn_upsampled_logits2 = ttnn_upsampled_logits2
        thres=0.5
        ...
        combined = np.concatenate([overlay_mask, overlay_ref, overlay_ttnn], axis=1)

        # Add labels for each overlay
        font = cv2.FONT_HERSHEY_SIMPLEX
        font_scale = 0.5
        thickness = 2
        label_colors = [(128, 0, 128), (0, 255, 0), (0, 0, 255)]  # White text
    ...
    logger.info(
        f"mean PCC values between Reference and ttnn model are {np.mean(pccs)}"
    )
